...
const MCQTest = ({ topic }: MCQTestProps) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-center p-8">
        ...
        <h3 className="text-lg font-bold dark:text-white">Gemini is preparing your test...</h3>
  ...
  if (testFinished) {
    ...
    return (
      <Card className="max-w-2xl mx-auto mt-8 border-none shadow-md bg-white dark:bg-[#161B2C]">
        <CardContent className="p-10 text-center">
          <div className={`h-20 w-20 rounded-full mx-auto flex items-center justify-center mb-6 ${
            percentage >= 70 ? 'bg-green-100 text-green-600' : 'bg-indigo-100 text-indigo-600'
          }`}>
            <CheckCircle2 size={40} />
          </div>
          <h2 className="text-3xl font-extrabold dark:text-white mb-2">Test Completed!</h2>
          <p className="text-slate-500 mb-8">You've finished the MCQ test on <span className="font-bold text-indigo-600">"{topic}"</span></p>
          ...
          <Button 
            ...
            className="w-full bg-indigo-600 hover:bg-indigo-700 h-12 text-lg font-bold gap-2"
          >
            <RefreshCcw size={20} />
            Try Again
          </Button>
  ...
  if (questions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-center p-8 bg-slate-50 dark:bg-[#161B2C] rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800">
        ...
        <p className="text-slate-500 text-sm mt-2 mb-8 max-w-md">
          Gemini can generate multiple-choice questions specifically for the topic of this study room.
        </p>
        <Button 
          onClick={generateQuestions} 
          className="bg-indigo-600 hover:bg-indigo-700 h-12 px-8 text-lg font-bold shadow-lg shadow-indigo-500/20"
        >
          Start MCQ Test
        </Button>
      </div>
    ...
    <div className="max-w-3xl mx-auto mt-4 space-y-6">
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-3">
          <Badge variant="outline" className="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-900/30">
            Question {currentIdx + 1} of {questions.length}
          </Badge>
          <span className="text-sm font-bold text-slate-400 uppercase tracking-widest">"{topic}"</span>
        </div>
      ...
      <Card className="border-none shadow-md bg-white dark:bg-[#161B2C] overflow-hidden">
        ...
        <CardContent className="space-y-3">
          {currentQ.options.map((option, idx) => {
            let variant = "outline";
            let className = "w-full justify-start text-left h-auto py-4 px-6 text-base font-medium transition-all duration-200 border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-700";
            ...
            if (selectedOption === idx) {
              if (showResult) {
                ...
                className += " bg-indigo-50 dark:bg-indigo-900/20 border-indigo-500 dark:border-indigo-500 text-indigo-700 dark:text-indigo-400 shadow-sm ring-1 ring-indigo-500";
              }
          ...
          {showResult && (
            <div className="mt-6 p-6 rounded-2xl bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/20 animate-in fade-in slide-in-from-bottom-2">
          ...
          <div className="pt-6 flex justify-end">
            {!showResult ? (
              <Button 
                ...
                className="bg-indigo-600 hover:bg-indigo-700 px-8 h-12 font-bold gap-2 shadow-lg shadow-indigo-500/20"
              >
                Submit Answer
              </Button>
            ) : (
